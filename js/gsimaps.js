...
GSI.TEXT.SHARE.DIALOG_LINK_MESSAGE = '次のURLをメール等で送付することで、現在表示されている地図を共有することができます。'+'<div style="font-size:85%;">※作図結果を反映した状態で共有したい場合は、本サイトの「名前をつけて一時保存」機能をご利用下さい</div>';
    ...
    HashSetProc : function(hash){
        ...
        if(n >= 1){
            ...
            if(this.vHashOptions != options){
                ...
                GSI.GLOBALS.onoffObjects[CONFIG.PARAMETERNAMES.JIHOKULINE ].obj.setVisible(viewSetting.latLngGrid );
                // 表示設定：UTMグリッド          vs=u[0/1]
                GSI.GLOBALS.onoffObjects[CONFIG.PARAMETERNAMES.LATLNGGRID ].obj.setVisible(viewSetting.utmGrid    );
                // 表示設定：磁北線               vs=j[0/1]
                GSI.GLOBALS.onoffObjects[CONFIG.PARAMETERNAMES.UTMGRID    ].obj.setVisible(viewSetting.jihokuLine );
